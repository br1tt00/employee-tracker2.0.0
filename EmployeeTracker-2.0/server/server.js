    ...
    inquirer
        .prompt([{
            ...
            choices: async () => {
                ...
                name: 'confirm',
                type: 'list',
                message: (answers) => `You are about to remove the ${answers.userSelectedDept}!`,
                choices: ['Yes', 'No']
            }
        ])
        .then((answers) => {
...
const updateRole = () => {
    ...
    .prompt([{
        ...
        choices: async () => {
            ...
            name: 'salaryUpdate',
            type: 'input',
            message: (answers) => `What is the new payment of ${answers.roleSelect}?`,
            when: (answers) => answers.whichPartOfRoleToUpdate === 'Salary'
        },{
            name:'confirm',
            type:'list',
            message: (answers) => `You would like to change the payment of ${answers.roleSelect} to ${answers.salaryUpdate}?`,
            choices: ['Yes', 'Cancel'],
            when: (answers) => answers.whichPartOfRoleToUpdate === 'Salary'
        },{
            ...
            if (answers.confirm === 'Cancel') {
                ...
                switch (answers.whichPartOfRoleToUpdate) {
                    case 'Title':
                        ...
                        break;
                    case 'Salary':
                        connection.query(`UPDATE role SET payment='${answers.salaryUpdate}' WHERE title='${answers.roleSelect}'`, (e, result) => {
                            if (e) throw e;
                            console.log(`\nSuccessfully updated payment of ${answers.roleSelect} to $${answers.salaryUpdate}.\n`);
                            startIntroQuestionaire();
                        })
                        break;
                    case 'Department':
